...
function Button({ children, onClick, variant = "lime", disabled, icon: Icon, type = "button" }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="sp-btn w-full py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 transition-opacity"
      style={{
...
function TextInput({ value, onChange, placeholder, mono: useMono, disabled }) {
  return (
    <input
      ...
      disabled={disabled}
      className="sp-input w-full rounded-2xl px-4 py-3.5 outline-none"
...
function CardFrame({ c, name, badge, onClick, href, children }) {
  ...
  const inner = (
    <>
      <div className="sp-video-card__top flex items-center justify-between p-3">
        <span className="sp-video-card__badge px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase" style={{ background: `${c}22`, border: `1px solid ${c}55`, color: c, ...body, letterSpacing: "0.04em" }}>
          Performance du défi
        </span>
        <div className="sp-video-card__play w-9 h-9 rounded-full flex items-center justify-center" style={{ background: `${c}22`, border: `1px solid ${c}66` }}>
          <Play size={14} color={c} fill={c} />
        </div>
      </div>
      <div className="p-3">
        <div className="sp-video-card__name" style={{ ...body, color: CHALK, fontWeight: 700, fontSize: 14 }}>🎥 Vidéo de {name}</div>
  ...
  if (onClick) {
    return (
      <button onClick={onClick} className="sp-video-card relative w-full h-40 rounded-2xl overflow-hidden flex flex-col justify-between text-left" style={{ ...commonStyle, cursor: "pointer" }}>
        {inner}
      </button>
    );
  }
  return (
    <a href={href || "#"} target="_blank" rel="noreferrer" className="sp-video-card relative w-full h-40 rounded-2xl overflow-hidden flex flex-col justify-between no-underline" style={commonStyle}>
      {inner}
    </a>
  );
}
        ...
        >
          ...
          {step === "auth" && (
            <>
              ...
              <div style={{ fontSize: 10, color: MUTE, textAlign: "center", ...body }}>
...
function HomePage({ nav, toast }) {
  ...
  const NavCard = ({ icon: Icon, title, subtitle, badge, gradient, onClick }) => (
    <button onClick={onClick} className="sp-nav-card rounded-3xl p-4 text-left relative overflow-hidden" style={{ background: gradient, border: "1px solid #FFFFFF14", cursor: "pointer", minHeight: 108 }}>
    ...
    <div className="max-w-2xl mx-auto w-full px-5 pb-10">
      ...
      <div className="flex flex-col gap-2">
        {ch.duels.map((d) => {
          ...
          return (
            <button key={d.id} onClick={() => nav("duel", { id, duelId: d.id })} className="sp-duel-row rounded-2xl p-3 flex items-center justify-between text-left" style={{ background: INK2, border: isMine ? `1px solid ${LIME}66` : "1px solid #FFFFFF10", cursor: "pointer" }}>
              <div>
...
function HallOfFamePage({ nav }) {
  ...
  return (
    <div className="max-w-2xl mx-auto w-full px-5 pb-10 flex flex-col gap-3">
      <div className="sp-search rounded-2xl flex items-center gap-2 px-3.5" style={{ background: INK2, border: "1px solid #FFFFFF14", height: 42 }}>
        <Search size={15} color={MUTE} />
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Chercher un défi ou un pseudo…"
          className="sp-search__input flex-1 bg-transparent outline-none"
      ...
      {winners && winners.length > 0 && (
        <div>
          ...
          <div className="sp-scroll-row flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
            {winners.map((w) => (
              <button
                key={w.duelId}
